...
//	private static List<PlayerThread> threads;
	...
	private static final int MAX_LOOP_VAL_PER_THREAD = 25;
	...
	public static void main(String[] args) throws ParseException {
		...
		try {
			...
			int maxLoops = server.total;
			System.out.println("ESTART!! el total de requests es: " + maxLoops);
			int threadsQty = getThreadsQty(maxLoops);
			
			ExecutorService executor = Executors.newFixedThreadPool(threadsQty);

			for (int i = 0; i < threadsQty; i++) {
				PlayerThread thread = new PlayerThread(server, players, MAX_LOOP_VAL_PER_THREAD,
						maxLoops, i);
				executor.execute(thread);
			}
			executor.shutdown();
			while(!executor.isTerminated()){}
			System.out.println("GG WP: plays = " + server.getPlays() + " calls = " + server.getCalls() );
			System.exit(0);
		} catch (Exception e) {
	...
	private static Options createOptions() {
		...
		OptionBuilder.withArgName(NAME_S);
		OptionBuilder.withLongOpt(NAME_L);
		OptionBuilder.hasArg();
		OptionBuilder.withDescription("Player name");
		OptionBuilder.isRequired(true);
		final Option name = OptionBuilder.create(NAME_S);
	...
	private static int getThreadsQty(int loop) {
		return Math.max(3, (int) Math.ceil((double) loop / MAX_LOOP_VAL_PER_THREAD));
	}
